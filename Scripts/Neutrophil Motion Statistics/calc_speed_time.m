...
% file that is generated from Imaris(R) and calculate their (normalised or not) 
% speed with time
...
% Select to normalise speed or not: 0 no, 1 yes
norm_factor = 1;
...
for exp_id = 1:num_exp
    ...
    for time_id = 1:frame_end-1
        ...
        u_time_cell = velocity_xyz_costh(time_int, time_id, dist_inn, ...
            dist_max, xx, yy, zz, num_cells, wound_x, wound_y, wound_z);
    ...
    %% Bin neutrophil speed with time (cell-based)
    ...
    for kk = 1:length(time_bins)-1
        ...
        speed_temp = u_time_all(time_bins(kk)+1:time_bins(kk+1),:);
        speed_temp = speed_temp(:);
        speed_cell{exp_id,kk} = speed_temp;
    end
...
%% Save data (*)
if norm_factor == 1
    save('norm_speed_time.mat', 'speed_cell', 'time');
else
    save('speed_time.mat', 'speed_cell', 'time');
end
